...
\label{sec:model-load-time}

Model load time vs size/connection setup/connection reuse cost.  Then
estimate how much time could be saved if resources would come from one
host or in one transfer, etc.


\subsection{Estimate possible data saving due to compression}
\label{sec:estim-poss-data}

Model compression ratio for mime types (different image
compressions?).  Then estimate how much data could be saved if these
resources would be indeed compressed.
